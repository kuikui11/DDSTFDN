...
import random
import numpy as np
from DenseGait_BaseOpenGait.utils.msg_manager import get_msg_mgr
        ...
        def sample_frames(seqs):
            ...
            if self.sampler in ['fixed', 'unfixed']:
                ...
                if self.ordered:
                    ...
                    # indices = np.random.choice(indices,1,replace=replace)
            ...
            for i in range(feature_num):
                ...
                # for j in index[:self.frames_all_limit] if self.frames_all_limit > -1 and len(indices) > self.frames_all_limit else index:
                #     sampled_fras[i].append(seqs[i][j])
            return sampled_fras
